...
export class CanalService {
  ...
  getCanalesPorcentaje(qs: string = "") {
    // let params = null;
    const headers = new HttpHeaders().set('Content-Type','aplication/json').set('Authorization', `${this.token}`);
    // headers.append('Authorization', `${this.token}`)
    // let options = new HttpO
    return this._http.get(`${this.url}/canal/canalesporcentaje${qs}`,{headers:headers});
  }
  delete(id:string, qs:string = ""){
    ...
    // let options = new HttpO
    return this._http.post(`${this.url}/canal/delete/${id}${qs}`,params,{headers:headers});
