...
// ,'paragraphFormat','alert'
 ...
 //...
  ...
  convertirAWhatsAppText(){
    ...
    // let parrafoTag = new RegExp(`<(p|span)[^>]*>[\s\S]*?<\/\1>|<\/?[^>]+>`,'g');
    // let parrafoTagFinal = new RegExp('</p>','g');
    // let spanTagInicial = new RegExp('<p>','g');
    // let spanTagFinal = new RegExp('</span>','g');
    let tagNBSP = new RegExp('&nbsp;','g');
    // let tagBr = new RegExp('<br>','g');
    this.campania.mensaje = this.campania.mensaje.replace(/(<([^>]+)>)/ig, '');
