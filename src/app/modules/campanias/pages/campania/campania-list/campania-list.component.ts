...
export class CampaniaListComponent implements OnInit {
  ...
  getNombreCanales(campania_canales){
    let canales_nombre = [];
    if(campania_canales.length > 0){
      campania_canales.forEach(campania_canal => {
        canales_nombre.push(campania_canal.canal.nombre);
      });
    }
   

    return canales_nombre.join(", ");
  }
  getNombreObjetivos(campania_objetivos){
    let objetivos = [];
    if(campania_objetivos.length > 0){
      campania_objetivos.forEach(campania_objetivo => {
        objetivos.push(campania_objetivo.objetivo.nombre);
      });
    }
